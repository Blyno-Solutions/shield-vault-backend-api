...
class SecurityService:
    def __init__(self, key: Optional[bytes] = None):
        if key is None:
            ...
            logger.warning("No encryption key provided. Generated a new key for development.")
        else:
            self.key = key
        self.fernet = Fernet(self.key)
    
    @classmethod
    def from_password(cls, password: str, salt: Optional[bytes] = None) -> 'SecurityService':
        if salt is None:
            salt = os.urandom(16)
